...
const getIncidents: ApiToolConfig = {
  name: "getIncidents",
  description:
    "Retrieves payment gateway incidents (scheduled and unscheduled downtimes) for a specified time period. Returns incident details including type, status, impact level, affected payment methods, and resolution times.",
  ...
  responseFormatter: (data) => {
    ...
    const formattedIncidents = incidents.map((incident: any) => ({
      ...
      affectedPaymentMethods:
        incident.incidentData?.detailsV2?.paymentGroups || [],
