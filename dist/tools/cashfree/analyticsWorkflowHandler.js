...
export async function handleAnalyticsByMerchantName(args) {
    try {
        ...
        const merchantData = await makeApiCall(CASHFREE_API_BASE_URL, "/commonmerchantsvc/merchants", merchantLookupPayload, "GET");
        ...
        if (merchants.length === 1) {
            ...
            const analyticsData = await makeApiCall(CASHFREE_API_BASE_URL, "/dexter-report/v1/router/analytics/transaction", analyticsPayload, "POST");
            if (!analyticsData) {
                return {
                    isError: true,
                    content: [{
